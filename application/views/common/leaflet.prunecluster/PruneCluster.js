// modified by GreenInfo Network to support the Leaflet labels
// look for the "GIN" tags
...
var PruneClusterForLeaflet = (L.Layer ? L.Layer : L.Class).extend({
    ...
    ProcessView: function () {
        ...
        for (i = 0, l = clusterCreationList.length; i < l; ++i) {
            ...
            //GIN add the label and trigger it as soon as we add the marker to the map
            //GIN and try to make up a left-offset that keeps 2-digit, 3-digit, and 4-digit numbers centered  (hopeless due to font size)
            //GIN the top positioning are is highly specific to this application, presuming upon a known global variable to tell us how tall the icon is
            if (icluster.population > 1) {
                var label = "" + icluster.population;

                var left = -10;
                if      (icluster.population >= 1000) left = -23;
                else if (icluster.population >= 100)  left = -18;
                else if (icluster.population >= 10)   left = -13;

                var top = BOTH_MARKER_HEIGHT / 2;

                creationMarker.bindLabel(label,{ noHide:true, offset:[left,top], zoomAnimation:false }).addTo(map).showLabel();
            } else {
...
var PruneClusterLeafletSpiderfier = (L.Layer ? L.Layer : L.Class).extend({
    ...
    Spiderfy: function (data) {
        ...
        //GIN: patch to support opacity and assignment; 0.7 has this method-chained, but 0.6 does not
        this._clusterMarker = data.marker;
        this._clusterMarker.setOpacity(0.3);
    },
